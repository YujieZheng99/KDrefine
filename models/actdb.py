...
class ACTripleDulplicateBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1,
                 deploy=False, nonlinear=None):
        super(ACTripleDulplicateBlock, self).__init__()
...
if __name__ == '__main__':
    ...
    for k, p in test_kernel_padding:
        tdb = ACTripleDulplicateBlock(C, O, kernel_size=k, padding=p, stride=1, deploy=False)
        tdb.eval()
