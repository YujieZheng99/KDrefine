...
def conv_bn(in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1):
    if CONV_BN_IMPL == 'base' or kernel_size == 1 or kernel_size >= 7:
        ...
        blk_type = ACBlock
    elif CONV_BN_IMPL == 'DDB':
        blk_type = DoubleDulplicateBlock
    elif CONV_BN_IMPL == 'TDB':
        blk_type = TripleDulplicateBlock
    elif CONV_BN_IMPL == 'ACTDB':
        blk_type = ACTripleDulplicateBlock
    elif CONV_BN_IMPL == 'ACDBB':
        blk_type = ACDBB
    else:
...
def switch_conv_bn_impl(block_type):
    assert block_type in ['base', 'DBB', 'ACB', 'DDB', 'TDB', 'ACDBB', 'ACTDB']
    global CONV_BN_IMPL
...
#     if arch == 'ResNet-18':
#         from represnet import create_Res18
#         model = create_Res18()
#     elif arch == 'ResNet-50':
#         from represnet import create_Res50
#         model = create_Res50()
#     elif arch == 'MobileNet':
