...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill == 'kdrefine':
            multiplier = 2
            logit_s = ((logit_s - torch.mean(logit_s, dim=1).unsqueeze(-1)) * multiplier) / (torch.std(logit_s, dim=1).unsqueeze(-1) + 1e-7)
            logit_t = ((logit_t - torch.mean(logit_t, dim=1).unsqueeze(-1)) * multiplier) / (torch.std(logit_t, dim=1).unsqueeze(-1) + 1e-7)
